	...
	function readRUL(input) {
		...
		if(input.files && input.files[0]) {
			...
			reader.onload = function(e) {
				$('#boardLinkText').attr('src', e.target.result);
			}
...
<form name="frmUpdate" method="post" enctype="multipart/form-data">
	<table class="boardViewTable">
		...
		<div id="originalImg">		
			...
			<input type="hidden" name="origimageFileName" value="${inquiry.ef_imageFileName}"/>		
			<!-- 외부 경로 이미지 불러오기 -->
			<div>[확대보기]</div>
			<img id="originalImage" usemap="#test" width="120" height="120" src="${contextPath}/inquiryimg/${inquiry.ef_i_no}/${inquiry.ef_imageFileName}"/>	
				<map name="test">
					<area shape="default" coords="10,8,150,292"
							href="${contextPath}/inquiryimg/${inquiry.ef_i_no}/${inquiry.ef_imageFileName}" target="_blank"/>
				</map>					
			</c:if>
		</div>
		<div id="previewImg" style="display:none;">	
			<img id="boardLinkText" width="120" height="120"/>
		</div>
					...
					<tr>
						...
						<td class="classModifyBtn">
							<c:if test="${inquiryCommentList.ef_id==ef_id}">
								<a href="${contextPath}/inquiry/deleteComment.do?ef_comment=${inquiryCommentList.ef_comment}&&ef_i_no=${inquiry.ef_i_no}" 
